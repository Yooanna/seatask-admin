...
async function loadProducts() {
    ...
    if (data.success) {
        document.getElementById('productsList').innerHTML = data.products.map(p => `
            <tr>
                <td><img src="${p.images?.[0] || ''}" width="50" height="50" style="object-fit:contain"></td>
                <td>${p.name}</td>
                <td>${p.category}</td>
                <td>RM ${p.variants?.[0]?.price || p.price}</td>
                <td><button class="edit-btn" onclick="alert('Edit: ${p.name}')">Edit</button><button class="delete-btn" onclick="alert('Delete: ${p.name}')">Delete</button></td>
            </tr>
        `).join('');
    }
}
    ...
    if (data.success) {
        document.getElementById('categoriesList').innerHTML = data.categories.map(c => `
            <div class="category-card">
                ...
                <button class="edit-btn" onclick="alert('Edit: ${c.name}')">Edit</button>
                <button class="delete-btn" onclick="alert('Delete: ${c.name}')">Delete</button>
            </div>
        `).join('');
    }
}
...
document.getElementById('addNewBtn')?.addEventListener('click', () => {
    alert('Add new product feature - will be implemented');
});
